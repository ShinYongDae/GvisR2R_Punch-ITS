...
void CGvisR2R_PunchView::SetMainMc(BOOL bOn)
{
	...
	else
	{
		if (m_pMpe)
			...
			//m_pMpe->Write(_T("MB440159"), 0); // 마킹부 Door Interlock OFF
	}
}
...
void CGvisR2R_PunchView::DoMark0()
{
	...
	case 15:
		...
		else
		{
			if (pDoc->WorkingInfo.LastJob.bUseJudgeMkHisto)
			{
				if (m_bFailMkJudge[0]) // when ChkRepunching(0)
				{
					if (IsMk0Done(m_nMkPcs[0], TRUE))
					{
						m_bFailMkJudge[0] = FALSE;
						// One more MK On Start....
						if (IsMk0Miss())
	...
	case 103:
		...
		{
			nSerial = m_nBufUpSerial[0]; // Cam0
			nTotMked = GetTotDefPcs0(nSerial);
...
void CGvisR2R_PunchView::DoMark1()
{
	...
	case 15:
		...
		else
		{
			if (pDoc->WorkingInfo.LastJob.bUseJudgeMkHisto)
			{
				if (m_bFailMkJudge[1]) // when ChkRepunching(1)
				{
					if (IsMk1Done(m_nMkPcs[1], TRUE))
					{
						m_bFailMkJudge[1] = FALSE;
						// One more MK On Start....
						if (IsMk1Miss())
	...
	case 103:
		...
		{
			nSerial = m_nBufUpSerial[1]; // Cam1
			nTotMked = GetTotDefPcs1(nSerial);
...
void CGvisR2R_PunchView::DoAutoChkShareVsFolder()	// 잔량처리 시 계속적으로 반복해서 이함수가 호출됨으로 좌우 마킹 인덱스 동일 현상 발생.(case AT_LP + 8:)
{
	...
	case AT_LP + (AtLpVsIdx::ChkShare) :
		...
		{
			if (IsShareVs())
				m_nStepAuto = AT_LP + (AtLpVsIdx::ChkShareVs);
		}
		break;
	...
	case AT_LP + (AtLpVsIdx::ChkShare) + 3:
		...
		m_nStepAuto = AT_LP + (AtLpVsIdx::ChkShare);
		break;
		...
		case MK_ST + (Mk2PtIdx::Start) + 1:
			if (pDoc->GetTestMode() == MODE_INNER || pDoc->GetTestMode() == MODE_OUTER)
			{
				if (!m_bTHREAD_UPDATE_REELMAP_UP && !m_bTHREAD_UPDATE_REELMAP_DN && !m_bTHREAD_UPDATE_REELMAP_ALLUP && !m_bTHREAD_UPDATE_REELMAP_ALLDN && !m_bTHREAD_MAKE_ITS_FILE_UP && !m_bTHREAD_MAKE_ITS_FILE_DN && !m_bTHREAD_UPDATE_REELMAP_INNER_UP && !m_bTHREAD_UPDATE_REELMAP_INNER_DN && !m_bTHREAD_UPDATE_REELMAP_INNER_ALLUP && !m_bTHREAD_UPDATE_REELMAP_INNER_ALLDN)
					//&& !m_bTHREAD_UPDATE_REELMAP_INOUTER_UP && !m_bTHREAD_UPDATE_REELMAP_INOUTER_DN && !m_bTHREAD_UPDATE_REELMAP_INNER_ALLUP && !m_bTHREAD_UPDATE_REELMAP_INNER_ALLDN) // Write Reelmap
	...
	if (m_bMkSt && IsBuffer())
	{
		...
		case MK_ST + (Mk2PtIdx::ChkSn) :
			if (m_nBufUpSerial[0] == m_nBufUpSerial[1])
			{
				...
				MsgBox(_T("상면의 좌/우 마킹 시리얼이 같습니다."));
				SetListBuf();
				m_nMkStAuto = MK_ST + (Mk2PtIdx::Start);
				break;
			}
			...
			if (bDualTest)
			{
				if (m_nBufDnSerial[0] == m_nBufDnSerial[1])
				{
					...
					pView->ClrDispMsg();
					//AfxMessageBox(_T("좌/우 마킹 시리얼이 같습니다."));
					MsgBox(_T("하면의 좌/우 마킹 시리얼이 같습니다."));
					SetListBuf();
					m_nMkStAuto = MK_ST + (Mk2PtIdx::Start);
					break;
				}
				...
				if (m_nBufUpSerial[0] != m_nBufDnSerial[0])
				{
					...
					pView->ClrDispMsg();
					//AfxMessageBox(_T("좌/우 마킹 시리얼이 같습니다."));
					MsgBox(_T("좌측 마킹의 상면과 하면 시리얼이 다릅니다."));
					SetListBuf();
					m_nMkStAuto = MK_ST + (Mk2PtIdx::Start);
					break;
				}
				...
				if (m_nBufUpSerial[1] && m_nBufDnSerial[1])
				{
					if (m_nBufUpSerial[1] != m_nBufDnSerial[1])
					{
						...
						pView->ClrDispMsg();
						//AfxMessageBox(_T("좌/우 마킹 시리얼이 같습니다."));
						MsgBox(_T("우측 마킹의 상면과 하면 시리얼이 다릅니다."));
						SetListBuf();
						m_nMkStAuto = MK_ST + (Mk2PtIdx::Start);
						break;
					}
...
BOOL CGvisR2R_PunchView::IsMk0Done(int nMkPcsIdx, BOOL bRemark)
{
	BOOL bDone = FALSE;

	if (pView->m_pVoiceCoil[0])
		bDone = pView->m_pVoiceCoil[0]->IsDoneMark(nMkPcsIdx, bRemark);

	return bDone;
}

BOOL CGvisR2R_PunchView::IsMk1Done(int nMkPcsIdx, BOOL bRemark)
{
	BOOL bDone = FALSE;

	if (pView->m_pVoiceCoil[1])
		bDone = pView->m_pVoiceCoil[1]->IsDoneMark(nMkPcsIdx, bRemark);
